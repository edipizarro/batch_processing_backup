...
def init_corrector(detections):
    ...
    corrector_detections = df2.dropDuplicates(subset=["candid", "oid"]).select(
        "oid",
        "aid",
        "candid",
        "dec",
...
def is_corrected_func(corrector_detections):
    ...
    corrector_detections = corrector_detections.withColumn(
        "corrected", when(condition, True).otherwise(False)
    )
    return corrector_detections
...
def is_first_corrected(corrector_detections):
    ...
    corrector_detections = corrector_detections.withColumn(
        "is_first_corrected",
        F.when(F.col("mjd") == F.min("mjd").over(window_spec), True).otherwise(False),
    )
...
def is_dubious_func(corrector_detections):
    corrector_detections = corrector_detections.withColumn(
        "is_negative",
        when(corrector_detections["isdiffpos"] == -1, True).otherwise(False),
    )
...
def is_stellar_func(corrector_detections):
    ...
    sharpnr_in_range_condition = (col("sharpnr") > SHARPNR_MIN) & (
        col("sharpnr") < SHARPNR_MAX
    )
    ...
    corrector_detections = (
        corrector_detections.withColumn(
            "near_ps1", when(condition_near_ps1, True).otherwise(False)
        )
...
def infinities_replacer(corrector_detections):
    ...
    columns_to_select = [
        col(column_name)
        for column_name in corrector_detections.columns
...
def correct(corrector_detections):
    ...
    corrector_detections = corrector_detections.withColumn(
        "aux1", pow(10, -0.4 * col("magnr").cast("float"))
    ).withColumn("aux2", pow(10, -0.4 * col("mag")))
    # Calculate 'aux3' column
    condition_aux3 = col("aux1") + col("isdiffpos") * col("aux2")
    corrector_detections = corrector_detections.withColumn(
        "aux3", when(condition_aux3 >= 0, condition_aux3).otherwise(0.0)
    )
    # Calculate 'mag_corr' column
    corrector_detections = corrector_detections.withColumn(
        "mag_corr", -2.5 * log10(col("aux3"))
    )
    # Calculate 'aux4' column
    corrector_detections = corrector_detections.withColumn(
        "aux4",
        (col("aux2") * col("e_mag")) ** 2
        - (col("aux1") * col("sigmagnr").cast("float")) ** 2,
    )
    # Calculate 'e_mag_corr' and 'e_mag_corr_ext' columns
    condition_aux4 = col("aux4") < 0
    e_mag_col = when(condition_aux4, float("inf")).otherwise(
        sqrt(col("aux4")) / col("aux3")
    )
    corrector_detections = corrector_detections.withColumn(
        "e_mag_corr", e_mag_col
    ...
    corrector_detections = (
        corrector_detections.withColumn(
            "mask", when(mask_condition_mag, True).otherwise(False)
        )
        .withColumn(
            "mag_corr",
            when(mask_condition_mag, float("inf")).otherwise(
                corrector_detections["mag_corr"]
            ),
        )
        .withColumn(
            "e_mag_corr",
            when(mask_condition_mag, float("inf")).otherwise(
                corrector_detections["e_mag_corr"]
            ),
        )
        .withColumn(
            "e_mag_corr_ext",
            when(mask_condition_mag, float("inf")).otherwise(
                corrector_detections["e_mag_corr_ext"]
            ),
        )
    )
    ...
    corrector_detections = (
        corrector_detections.withColumn(
            "mask", when(mask_condition_e_mag, True).otherwise(False)
        )
        .withColumn(
            "e_mag_corr",
            when(mask_condition_e_mag, float("inf")).otherwise(
                corrector_detections["e_mag_corr"]
            ),
        )
        .withColumn(
    ...
    corrector_detections = corrector_detections.drop(
        "aux1", "aux2", "aux3", "aux4", "mask"
    )
    # Sort columns
...
def restruct_extrafields(corrector_detections):
    columns_not_extrafields_fp_corrected = [
        "pid",
        "oid",
        "mjd",
        "fid",
    ...
    columns_to_nest_fp_corrected = [
        col
        for col in corrector_detections.columns
        if col not in columns_not_extrafields_fp_corrected
    ]
    nested_col_fp_corrected = struct(*columns_to_nest_fp_corrected)
    fp_corrected = corrector_detections.select(
        "*", nested_col_fp_corrected.alias("extra_fields")
    )
    fp_corrected = fp_corrected.select(
        "adpctdif1",
        "adpctdif2",
        "chinr",
        "clrcoeff",
    ...
    columns_to_nest = [
        col
        for col in corrector_detections.columns
        if col not in columns_not_extrafields
    ]
    nested_col = struct(*columns_to_nest)
    corrector_detections = corrector_detections.select(
        "*", nested_col.alias("extra_fields")
    )
    corrector_detections = corrector_detections.select(
        "aid",
        "candid",
...
def create_weighted_columns(non_forced):
    non_forced = non_forced.withColumn(
        "weighted_e_ra", 1.0 / (col("e_ra_arcsec") ** 2.0)
    )
    non_forced = non_forced.withColumn(
        "weighted_e_dec", 1.0 / (col("e_dec_arcsec") ** 2.0)
    )
    return non_forced
...
def correct_coordinates(non_forced):
    ...
    non_forced = (
        non_forced.withColumn(
            "weighted_sum_ra",
            F.sum(F.col("ra") * F.col("weighted_e_ra")).over(window_spec),
        )
        .withColumn(
            "weighted_sum_dec",
            F.sum(F.col("dec") * F.col("weighted_e_dec")).over(window_spec),
        )
    ...
    corrected_coords = (
        non_forced.withColumn(
            "meanra", col("weighted_sum_ra") / col("total_weight_e_ra")
        )
...
def execute_corrector(lightcurve_df):
    ...
    return (
        ...
        candids,
        forced_photometries,
        detections_non_forced_corrected,
    )
...
def produce_correction(lightcurve_df):
    (
        ...
        forced_photometries,
        detections_non_forced_corrected,
    ) = execute_corrector(lightcurve_df)
    print("Preparing output (joining results)...")
    oid_detections_df = corrector_detections.groupBy("oid").agg(
        collect_list(struct(corrector_detections.columns)).alias("detections")
    )
    non_detections = non_detections.groupBy("oid").agg(
        collect_list(struct(non_detections.columns)).alias("non_detections")
    )
    ...
    correction_step_output = correction_step_output.join(
        non_detections, on="oid", how="left"
    )
    correction_step_output = correction_step_output.select(
        *[
            col(column_name)
            for column_name in correction_step_output.columns
            if column_name != "non_detections"
        ],
        when(col("non_detections").isNotNull(), col("non_detections"))
        .otherwise(array())
        .alias("non_detections")
    )
