...
def create_non_detections_frame(df):
    ...
    # For testing purposes only, we count the number of non-detections and the number of unique non-detections
    non_detection_count = unnested_non_detections.count()
    unique_non_detection_count = unique_non_detections.count()
    diff_non_detection_count = non_detection_count - unique_non_detection_count
    print('The number of non-detections was: ', str(non_detection_count))
    print('The number of unique non-detections was: ', str(unique_non_detection_count))
    print('The number of non-detections dropped was: ', str(diff_non_detection_count)) 
...
def create_forced_photometries_frame(df):
    ...
    print('The number of forced photometries was: ', unnested_fp .count())
    print('The number of unique forced photometries was: ', unique_fp .count())
    print('The number of dropped forced photometries was: ', unnested_fp .count() - unique_fp .count())
    return unique_fp 
...
def create_detections_frame(df):
    ...
    *[col('extra_fields.' + field) for field in unique_detections.select('extra_fields.*').columns] +
    ...
    print('The number of alerts was: ', alert_df.count())
    print('The number of detections was: ', unnested_detections.count())
    print('The number of unique detections was: ', unique_detections.count())
    print('The number of joined detections was: ', join_alertas_detections.count())
    print('The number of joined extra fields was: ', join_extra_fields.count())
    print('Detections dataframe has a number of rows: ', detections_dataframe.count())

    return detections_dataframe


parquets_dataframe = load_dataframes('data/*')
non_detection_frame = create_non_detections_frame(parquets_dataframe)
detections_frame = create_detections_frame(parquets_dataframe)
forced_photometries_frame = create_forced_photometries_frame(parquets_dataframe)
non_detection_frame.write.parquet("output/non_detection.snappy.parquet", compression = 'snappy', mode='overwrite')
detections_frame.write.parquet("output/detections.snappy.parquet", compression = 'snappy', mode='overwrite')
forced_photometries_frame.write.parquet("output/forced_photometries.snappy.parquet", compression = 'snappy', mode='overwrite')
