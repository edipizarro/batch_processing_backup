...
def modify_alert_fields(df):
    ...
    expanded_extra_fields = df.select("*",col("extra_fields.*"))
    expanded_extra_fields = expanded_extra_fields.drop('extra_fields').drop('prv_candidates').drop('fp_hists')
...
def expand_fields_adding_alert_data(df):
    unnested_extra_fields = df.select("*", col("extra_fields.*"))
...
def parse_extra_fields(df, type_of_detection):
    ...
    not_extrafields_withid =  not_extrafields[:]
    not_extrafields_withid.remove("oid")
    if type_of_detection!="non_detection":
        not_extrafields_withid.remove("candid")
...
def calculate_mag(fp_data):
    ...
    mask_forcediffimflux = isclose(F.col("modified_forcediffimflux"),_ZERO_MAG_condition_col)
    mask_forcediffimfluxunc = isclose(F.col("modified_forcediffimfluxunc"),_ZERO_MAG_condition_col)
