...
from pyspark.sql.functions import col, count, explode, concat, sum, sqrt, min, max, mean, expr, when, stddev_pop
...
def separate_dataframe(directory):
    parquets_correction = load_dataframes(directory)
    detections = parquets_correction.select("detections")
    exploded_dets = detections.select(explode("detections").alias("exploded_data"))
    detections_df = exploded_dets.select("exploded_data.*")
    non_detections = parquets_correction.select("non_detections")
...
def calculate_object_stats(detections):
    ...
    joined_df = dets_num.join(mjds, on="oid").join(corrected_coordinates, on="oid").join(oid_fid_sid, on="oid").join(stellar_corrected, on='oid')
    return joined_df
...
def execute_step_magstats(directory):
    detections, non_detections = separate_dataframe(directory)
    ...
    return object_stats, magstats

object_stats, magstats = execute_step_magstats("correction_step.snappy.parquet")
object_stats.show()
magstats.sort('oid').show()